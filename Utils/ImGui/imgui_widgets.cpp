...
bool ImGui::CustomButtonEx(const char* label, const char* name, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 pos = window->DC.CursorPos + ImVec2(8, 0);
    ...
    //it_anim->second.rect = ImLerp(it_anim->second.rect, pressed ? ImColor(157, 131, 216, 20) : hovered ? ImColor(157, 131, 216) : ImColor(100, 84, 138), g.IO.DeltaTime * 6.f);
    it_anim->second.rect = ImLerp(it_anim->second.rect, pressed ? ImColor(25, 25, 25, 20) : hovered ? ImColor(25, 25, 25, 255) : ImColor(20, 20, 20, 255), g.IO.DeltaTime * 6.f);
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    /* frames it with a white glow
    ImGui::GetWindowDrawList()->AddShadowRect(
        bb.Min,
        bb.Max,
        ImColor(255.f, 255.f, 255.f, 0.2f), 20, ImVec2(0, 0), 1, 10
    );
    */
    window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.rect), 4);
...
{
    return CustomButtonEx(label, label, size_arg, flags);
}
...
bool ImGui::Tab(bool selected, const char* icon, const char* label, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    const ImRect bb(pos, pos + size);
    const ImRect icon_bb(pos + ImVec2(5, (size_arg.y - 21) / 2), pos + ImVec2(26, (size_arg.y + 21) / 2));
    ...
    it_anim->second.backgroundOpacity = ImLerp(it_anim->second.backgroundOpacity, selected ? 0.8f : (hovered ? 0.5f : 0.0f), g.IO.DeltaTime * 10.f);
    it_anim->second.icons = ImLerp(it_anim->second.icons, selected ? ImColor(157, 131, 216) : hovered ? ImColor(157, 131, 216, 200) : ImColor(34, 32, 34), g.IO.DeltaTime * 10.f);
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? ImColor(255, 255, 255) : hovered ? ImColor(255, 255, 255, 200) : ImColor(60, 58, 60), g.IO.DeltaTime * 10.f);


    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, ImColor(20 / 255.f, 18 / 255.f, 20 / 255.f, it_anim->second.backgroundOpacity), 3);
    GetWindowDrawList()->AddText(bb.Min + ImVec2(8, (size_arg.y - CalcTextSize(icon).y) / 2), GetColorU32(it_anim->second.icons), icon);
    GetWindowDrawList()->AddText(bb.Min + ImVec2(30, (size_arg.y - CalcTextSize(label).y) / 2), GetColorU32(it_anim->second.text), label);
...
bool ImGui::SubTab(bool selected, const char* label, const char* name, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? ImColor(255, 255, 255) : hovered ? ImColor(60, 58, 60, 200) : ImColor(60, 58, 60, 160), g.IO.DeltaTime * 10.f);
    it_anim->second.rect = ImLerp(it_anim->second.rect, selected ? ImColor(20, 18, 20) : hovered ? ImColor(20, 18, 20, 200) : ImColor(18, 16, 18, 180), g.IO.DeltaTime * 10.f);

    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.rect), 4.f);
    GetWindowDrawList()->AddText(bb.Min + ImVec2(size_arg.x - 36 - CalcTextSize(name).x / 2, (size_arg.y - CalcTextSize(name).y) / 2), GetColorU32(it_anim->second.text), name);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.selectedRect = ImLerp(it_anim->second.selectedRect, *v ? 1.0f : (hovered ? 0.4f : 0.0f), 0.2f * (0.6f - ImGui::GetIO().DeltaTime));

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));
    ImU32 check_col = GetColorU32(ImGuiCol_CheckMark);

    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(20, 18, 20, 255), 2);

    const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));

    window->DrawList->AddShadowCircle(check_bb.Min, 7.f, ImColor(157 / 255.f, 131 / 255.f, 216 / 255.f, it_anim->second.selectedRect), 35.f, ImVec2(10, 10));

    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(157 / 255.f, 131 / 255.f, 216 / 255.f, it_anim->second.selectedRect), 3);

    RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad + 5, pad + 5), ImColor(255 / 255.f, 255 / 255.f, 255 / 255.f, it_anim->second.selectedRect), square_sz - pad * 5.0f);

    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y);

    if (label_size.x > 0.0f)
        window->DrawList->AddText(label_pos, ImColor(60, 58, 60), label);

    //active
    window->DrawList->AddText(label_pos, ImColor(255 / 255.f, 255 / 255.f, 255 / 255.f, it_anim->second.selectedRect), label);
...
bool ImGui::CustomSliderScalar(const char* label, const char* name, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 label_size = CalcTextSize(name, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(114, 2), window->DC.CursorPos + ImVec2(94, 2) + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect frame_dd(window->DC.CursorPos + ImVec2(2, 6), window->DC.CursorPos + ImVec2(-168, 6) + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? ImColor(255, 255, 255) : hovered ? ImColor(255, 255, 255) : ImColor(128, 128, 128), g.IO.DeltaTime * 6.f);

    GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 8), frame_bb.Max - ImVec2(12, 6), ImColor(20, 18, 20), 100.f);
    ...
    it_anim->second.slow_anim = ImLerp(it_anim->second.slow_anim, grab_bb.Min.x - frame_bb.Min.x, g.IO.DeltaTime * 14.f);
    GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 8), ImVec2(frame_bb.Min.x + it_anim->second.slow_anim, grab_bb.Max.y - 4), ImColor(157, 131, 216), 100.f);
    ImGui::GetWindowDrawList()->AddShadowRect(frame_bb.Min + ImVec2(0, 8), ImVec2(frame_bb.Min.x + it_anim->second.slow_anim, grab_bb.Max.y - 4), ImColor(90.f, 0.f, 120.f, 0.2f), 40.0f, ImVec2(0, 0), 1, 10);

    auto label_buffer_size = CalcTextSize(value_buf, value_buf_end, true);

    GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - 14 - label_buffer_size.x, frame_bb.Min.y - 10), GetColorU32(it_anim->second.text), value_buf, value_buf_end);
    GetWindowDrawList()->AddText(frame_dd.Min + ImVec2(8, -4), GetColorU32(it_anim->second.text), name);
...
{
    return CustomSliderScalar(label, label, data_type, p_data, p_min, p_max, format, flags);
}
...
bool ImGui::CustomSliderFloat(const char* label, const char* name, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    return CustomSliderScalar(label, name, ImGuiDataType_Float, v, &v_min, &v_max, format, flags);
}
...
bool ImGui::CustomSliderInt(const char* label, const char* name, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags)
{
    return CustomSliderScalar(label, name, ImGuiDataType_S32, v, &v_min, &v_max, format, flags);
}
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ...
    return InputTextEx(label, NULL, buf, (int)buf_size, ImVec2(0.0f, 0.0f), flags, callback, user_data);
}
