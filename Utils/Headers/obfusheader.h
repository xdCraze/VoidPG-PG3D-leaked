...
namespace obf
{
    ...
    {
        thread_local T v = value; return v;
    }

    template <typename T, T value>
    static constexpr T ensure_constexpr()
    {
        return value;
    }
    ...
    volatile void obf_draw_orig(const char* param)
    {
    } // to avoid crashing we assign a real func
    ...
    volatile void main_decoy()
    {
        ...
        WATERMARK("Stop reversing the program (Youre a complete loser)",
...
#endif
// EXTENSION
/*uintptr_t StringToInt(const char* c)
{
    int out = std::stoi(c);
    if(out > 0)
    {
...
// TOO MUCH JUST FOR A NUMBER LOLOLOL (imma do it later)
#ifndef NO_OBFUSCATE
#define OBFINT(num) ((std::function<decltype(num)()>) [](){if(std::stoi(OBF(#num)) > 0) return (decltype(num))std::stoi(OBF(#num));return (decltype(num))strtoul(OBF(#num), nullptr, 16);})()
#define OBF_TRUE OBFINT(1)
#define OBF_FALSE OBFINT(0)
#else
#define OBFINT(num) num
#define OBF_TRUE true
#define OBF_FALSE false
#endif // !NO_OBFUSCATE
