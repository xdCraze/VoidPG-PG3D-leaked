...
public:
    ...
    float ToastSize = 350.0;
    float ToastWidth = 80.0;
    float YAddPos = 85.0;
...
namespace ImGui
{
    ...
    inline void RenderNotifications()
    {
        ...
        ImGui::PushStyleColor(ImGuiCol_Button, ImColor(0, 0, 0, 128).Value);
        ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImColor(0, 0, 0, 180).Value);
        ...
        for (size_t i = 0; i < notifications.size(); ++i)
        {
            ...
            */
            ...
            ImGui::SetNextWindowPos(ImVec2(ImGui::GetIO().DisplaySize.x - 70, ImGui::GetIO().DisplaySize.y - ((i + 1) * currentToast->YAddPos)), ImGuiCond_Always, ImVec2(1.0f, 1.0f));
            ImGui::SetNextWindowSize(ImVec2(currentToast->ToastSize, currentToast->ToastWidth));
            ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, 10.0f); // Set the rounding radius (10.0f is the amount of rounding)
            Begin(windowName, nullptr, currentToast->getWindowFlags() | ImGuiWindowFlags_NoMove);
            ...
            // Here we render the toast content
            {
                ...
                if (NOTIFY_USE_DISMISS_BUTTON)
                {
                    ...
                    if (currentToast->ShowRemoveButton) {
