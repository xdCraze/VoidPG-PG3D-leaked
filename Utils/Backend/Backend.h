...
public:
	...
	static DWORD WINAPI AuthActivity(LPVOID lpReserved); // Checks all 20 Seconds if User and Pass is still valid, if it is, it'll crash game

	static DWORD WINAPI DetectHooks(LPVOID lpReserved); // Checks all 2 seconds if any Hooks are found (ours are whitelisted)
public:
